...
void searchAndGet(char *filename)
{
     int isExists = 0;
     fp = fopen(filename, "r+");
     fflush(stdin);
     printf("\t\tEnter the service name you want to search: ");
     scanf("%s", servicename);
     rewind(fp);
     while (fread(&s1, recsize, 1, fp) > 0 && found == 0)
     {
          ...
          {
               isExists = 1;
               printf("Service Name : %s",s1.name);
               printf("Service Code : %s",s1.code);
               printf("\nService Cost : %s",s1.price);
​
          }
     }
}
...
​
     ...
     case '4':
          ...
          break;
     case '5':
          searchAndGet(service_file_name);
     case '6':
          fclose(fp);
          exit(0);
     }
